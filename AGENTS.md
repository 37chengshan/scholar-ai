...
5. 需要真实视觉级点击、滚动、菜单、遮挡、响应式验证时，优先使用 `Computer Use` 作为 `Chrome DevTools MCP` 的补充；需要脚本化浏览器自动化时，优先使用 `browser-harness`。
6. `browser-use` 已弃用，禁止作为常规或备用工具；历史 runbook 中若仍出现 `browser-use`，仅作归档参考，不得作为新工作的执行路径。
