        ...
        def wrapper(*args, **kwargs):
            for attempt in range(max_retries):
                try:
                    ...
                    # Intentional synchronous sleep: MilvusService methods are
                    # synchronous (pymilvus is a sync-only library) and must be
                    # called via run_in_executor from async routes. Inside an
                    # executor thread, time.sleep is safe and does NOT block
                    # the event loop.
                    time.sleep(delay)
            return None
...
class MilvusService:
    ...
    def initialize_collections(self) -> None:
        ...
        if self.has_collection(settings.MILVUS_COLLECTION_CONTENTS):  # "paper_contents"
            logger.warning(
                "Deleting old paper_contents collection (1024-dim BGE-M3 data)"
            )
            self.drop_collection(settings.MILVUS_COLLECTION_CONTENTS)
            logger.info(
                "Old paper_contents collection deleted, all BGE-M3 data removed"
            )
    ...
    def _create_index(
        self, collection: Collection, field_name: str = "embedding"
    ) -> None:
    ...
    def create_paper_images_collection(self) -> Collection:
        ...
        fields = [
            ...
            FieldSchema(
                name="page_num", dtype=DataType.INT32, description="Page number"
            ),
            FieldSchema(
                name="caption",
                dtype=DataType.VARCHAR,
            ...
            FieldSchema(
                name="bbox", dtype=DataType.JSON, description="Bounding box coordinates"
            ),
        ]
        ...
        logger.info("Created paper_images collection", embedding_dim=self.embedding_dim)
        return collection
    ...
    def create_paper_tables_collection(self) -> Collection:
        ...
        logger.info("Created paper_tables collection", embedding_dim=self.embedding_dim)
        return collection
    ...
    def create_collections(self) -> Dict[str, Collection]:
        ...
        try:
            collections["paper_images"] = Collection(
                settings.MILVUS_COLLECTION_IMAGES, using=self._alias
            )
        ...
        try:
            collections["paper_tables"] = Collection(
                settings.MILVUS_COLLECTION_TABLES, using=self._alias
            )
        ...
        try:
            collections["paper_contents"] = Collection(
                settings.MILVUS_COLLECTION_CONTENTS, using=self._alias
            )
    ...
    def drop_collection(self, collection_name: str) -> None:
        ...
        if self.has_collection(collection_name):
            ...
            logger.warning(
                f"Collection {collection_name} does not exist, skip dropping"
            )
    ...
    def create_collection_v2(self) -> None:
        ...
        fields = [
            ...
            FieldSchema(
                name="content_data", dtype=DataType.VARCHAR, max_length=32000
            ),  # Increased from 8000 to 32000
            FieldSchema(name="raw_data", dtype=DataType.JSON),
            FieldSchema(
                name="embedding", dtype=DataType.FLOAT_VECTOR, dim=self.embedding_dim
            ),
            # Per D-09: New fields for embedding failure handling
            FieldSchema(name="indexable", dtype=DataType.BOOL, default_value=True),
        ...
        collection = Collection(
            settings.MILVUS_COLLECTION_CONTENTS_V2, schema, using=self._alias
        )
            ...
            while retries < max_retries:
                try:
                    ...
                    for idx, item in enumerate(batch):
                        ...
                        # Per D-09: Skip failed embeddings, don't use zero-vector
                        if not embedding or all(v == 0.0 for v in embedding):
                            logger.warning(
                                "Skipping embedding-failed item",
                        ...
                        # Build entity with indexable=True (default) per D-09
                        entity = {
                            "paper_id": item["paper_id"],
                            "user_id": item["user_id"],
                            "content_type": item.get("content_type", "text"),
                            ...
                            "section": item.get("section")
                            or "",  # Fix: Handle explicit None values
                            ...
                            "content_data": str(
                                item.get("content_data", item.get("text", ""))
                            )[:8000],
                            "raw_data": item.get("raw_data", {}),
                            "embedding": embedding,
    ...
    def _build_expr_from_constraints(
        ...
        if year_paper_ids:
            if effective_paper_ids:
                effective_paper_ids = list(
                    set(effective_paper_ids) & set(year_paper_ids)
                )
            else:
        ...
        if effective_paper_ids:
            if len(effective_paper_ids) > 100:
                logger.warning(
                    f"Too many paper_ids ({len(effective_paper_ids)}), limiting expr"
                )
