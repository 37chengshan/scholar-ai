...
export function SearchSidebar({
  ...
  return (
    <motion.div
      ...
      className="w-[220px] border-r border-border/70 flex flex-col h-full bg-paper-2 flex-shrink-0"
    >
      <div className="px-5 py-4 border-b border-border/60 bg-paper-1/80 backdrop-blur-md sticky top-0 z-10 flex justify-between items-center">
        <h2 className="font-serif text-xl font-semibold tracking-tight leading-none mb-1">{labels.sources}</h2>
        <p className="text-[9px] font-semibold tracking-widest text-primary">{labels.global}</p>
      </div>

      <div className="flex-1 overflow-y-auto py-5 flex flex-col gap-6 px-4">
        <div>
          <div className="editorial-rule-heading mb-3 px-1">{labels.aggregators}</div>
          <button
          ...
          >
            ...
            <span className="text-[11px] font-semibold tracking-wide flex-1 text-left">{labels.allSources}</span>
        ...
        <div>
          <div className="editorial-rule-heading mb-3 px-1">{labels.connectors}</div>
          <div className="flex flex-col gap-0.5">
            {sources.map((source) => (
              ...
              >
                ...
                />
                <span className="text-[11px] font-semibold tracking-wide flex-1 text-left truncate">{source.name}</span>
                {source.results > 0 && (
