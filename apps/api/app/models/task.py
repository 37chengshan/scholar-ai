...
class ProcessingTask(Base):
    ...
    task_type: Mapped[str] = mapped_column(String(50), default="pdf_processing")
    ...
    failure_stage: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)
    ...
    retry_trace_id: Mapped[Optional[str]] = mapped_column(String(36), nullable=True)
    cost_breakdown: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    cache_stats: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
    ...
    import_jobs: Mapped[List["ImportJob"]] = relationship("ImportJob", back_populates="processing_task")
    ...
    __table_args__ = (
        ...
        Index("idx_processing_tasks_task_type", "task_type"),
    )
...
class ParseTask(Base):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    batch_task_id: Mapped[Optional[str]] = mapped_column(ForeignKey("batch_tasks.id"), nullable=True)
    paper_id: Mapped[Optional[str]] = mapped_column(ForeignKey("papers.id"), nullable=True)
    filename: Mapped[str] = mapped_column(String, nullable=False)
    stage: Mapped[ParseTaskStage] = mapped_column(SQLEnum(ParseTaskStage), default=ParseTaskStage.PENDING)
