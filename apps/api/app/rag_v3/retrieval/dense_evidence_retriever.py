...
class DenseEvidenceRetriever:
    ...
    def retrieve(self, query: str, top_k: int, paper_id_filter: list[str] | None = None) -> list[EvidenceCandidate]:
        if not self._embedding_provider or not self._collection_name:
            return []
        try:
            return self._milvus_search(query=query, top_k=top_k, paper_id_filter=paper_id_filter)
        except Exception:
            if paper_id_filter:
                # Retry without filter as fallback
                try:
                    return self._milvus_search(query=query, top_k=top_k, paper_id_filter=None)
                except Exception:
                    return []
            return []

    def _milvus_search(self, query: str, top_k: int, paper_id_filter: list[str] | None = None) -> list[EvidenceCandidate]:
