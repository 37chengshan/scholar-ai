...
migrated_model_files=(
  "backend-python/app/models/note.py"
  "backend-python/app/models/session.py"
  "backend-python/app/models/rag.py"
)
...
if rg -n "db\.(execute|add|delete|flush|refresh|commit)|\bselect\(|func\.count|text\(" backend-python/app/api/papers/paper_crud.py -S >/dev/null; then
...
if [[ -f backend-python/app/api/search.py ]]; then
  if ! rg -n "deprecated|DEPRECATED" backend-python/app/api/search.py -S >/dev/null; then
...
(
  cd backend-python
  ./venv_new/bin/pytest -q tests/unit/test_services.py --maxfail=1
  ./venv_new/bin/pytest -q tests/test_unified_search.py --maxfail=1
)
echo "✓ 后端关键测试通过"
