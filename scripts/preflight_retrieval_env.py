...
def _check_milvus(embedding: list[float]) -> dict[str, Any]:
    ...
    if has_collection:
        smoke_results = milvus.search_contents_v2(
            ...
            content_type="text",
            top_k=1,
            constraints=SearchConstraints(user_id="preflight-user"),
        )
...
def _check_qdrant(embedding: list[float]) -> dict[str, Any]:
...
def main() -> int:
    ...
    try:
        embedding_check = _check_embedding()
    ...
    try:
        if args.backend == "qdrant":
            checks.append(_check_qdrant(embedding))
        else:
            checks.append(_check_milvus(embedding))
    except Exception as exc:
        checks.append(
            {
                "name": "vector_store",
