...
refs=$( (rg -n "app/hooks/useKnowledgeBases" frontend/src -S || true) | wc -l | tr -d ' ')
if [[ "$refs" -ne 0 ]]; then
...
conflicts=$(comm -12 <(ls frontend/src/hooks 2>/dev/null | sort) <(ls frontend/src/app/hooks 2>/dev/null | sort) || true)
if [[ -n "$conflicts" ]]; then
...
if rg -n "return \{ success: true, data:" frontend/src/services/kbApi.ts -S >/dev/null; then
