...
- `docs/specs/governance/migration-conditions.md` 已定义：只有在 Phase 0~5 验收全部通过后，才允许进入物理迁移执行阶段。
...
- `docs/specs/governance/migration-conditions.md`
...
处理规则：
- `docs/plans/**`、`docs/specs/adr/**` 可保留历史路径说明
- 其他位置全部改为新路径，或删除旧引用
...
--glob '!docs/plans/**' --glob '!docs/specs/adr/**'
```
