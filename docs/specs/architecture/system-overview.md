...
- SSE 事件命名、载荷与完成信号遵循 docs/specs/architecture/api-contract.md。
...
- Dashboard 主入口固定为 `apps/web/src/app/pages/Dashboard.tsx`，只负责展示连续研究状态、阻塞项与下一步跳转，不承担执行逻辑。
- Chat 前端主入口固定为 `apps/web/src/features/chat/workspace/ChatWorkspaceV2.tsx`。
- Chat 页负责统一 runtime 执行；Search、KB、Read、Notes、Compare、Review 只负责输入准备、状态解释、结果展示与深链跳转，不再各自维护并行执行内核。
- Search -> KB -> Read/Chat -> Notes/Compare/Review 是前端唯一主研究链；跨页继续进入 Chat 时，durable scope 仅保留在 URL query（`paperId` / `kbId` / `paper_ids` / `runId`），一次性 prompt 与 evidence 通过 `location.state.handoff` 传递，且只预填不自动发送。
...
- 引入新运行时依赖：同步更新 docs/specs/domain/resources.md。
- 改动 SSE 行为：同步更新 docs/specs/architecture/api-contract.md。
