...
颜色、字体、阴影、半径与布局变量以 `tokens/` 目录下的 CSS 文件为唯一真源。Figma 稿、文档、组件实现都必须回对该文件。

Source of Truth 文件：
- `apps/web/src/styles/tokens/color.css` -- 颜色、功能语义令牌
- `apps/web/src/styles/tokens/typography.css` -- 字体、字号、行高、字重
- `apps/web/src/styles/tokens/spacing.css` -- 间距、布局间距
- `apps/web/src/styles/tokens/motion.css` -- 动效时长、缓动、意图令牌
- `apps/web/src/styles/tokens/elevation.css` -- 阴影、圆角、边框宽度
- `apps/web/src/styles/theme.css` -- @theme 块，桥接令牌到 Tailwind
- `apps/web/src/styles/fonts.css` -- 字体导入

#### 2.1 Core Colors (oklch)

颜色基于三大家族：paper（暖中性背景）、ink（冷中性文字）、accent（品牌橙）。

- `--color-primary: var(--accent-600)` -> `oklch(0.555 0.195 40)`
- `--color-primary-foreground: oklch(0.985 0.005 80)`
- `--color-secondary: var(--accent-400)` -> `oklch(0.705 0.185 45)`
- `--color-background: var(--paper-50)` -> `oklch(0.985 0.005 80)`
- `--color-surface: oklch(1 0 0)`
- `--color-card: oklch(1 0 0)`
- `--color-muted: var(--paper-200)` -> `oklch(0.935 0.012 75)`
- `--color-muted-foreground: var(--ink-500)` -> `oklch(0.485 0.012 238)`
- `--color-foreground: var(--ink-900)` -> `oklch(0.145 0.004 230)`
- `--color-accent-foreground: var(--accent-600)`
- `--color-destructive: oklch(0.545 0.220 25)`
- `--color-border: oklch(0.885 0.006 75 / 0.6)`

#### 2.2 Support Tokens

- `--color-ring: var(--accent-500)` -> `oklch(0.625 0.205 42)`
- `--color-chart-1` through `--color-chart-5`: oklch values in `tokens/color.css`
- Status tokens: `--color-status-completed`, `--color-status-processing`, `--color-status-failed`, `--color-status-pending` (each with `-bg` variant)
- Evidence tokens: `--color-evidence-strong`, `--color-evidence-moderate`, `--color-evidence-weak`, `--color-evidence-contradictory`, `--color-evidence-unsupported`
...
**Shadow Scale (soft):** `--shadow-xs` through `--shadow-2xl`, `--shadow-inset`

**Paper Shadows (neobrutalist):**
- `--shadow-paper: 4px 4px 0px 0px oklch(0.15 0 0)`
- `--shadow-paper-hover: 6px 6px 0px 0px oklch(0.15 0 0)`
- `--shadow-paper-active: 2px 2px 0px 0px oklch(0.15 0 0)`

**Component Shadows (neobrutalist):**
- `--shadow-neo: 3px 3px 0px 0px var(--ink-900)` -- default button/input
- `--shadow-neo-hover: 4px 4px 0px 0px var(--ink-900)` -- hover
- `--shadow-neo-active: 1px 1px 0px 0px var(--ink-900)` -- active/pressed
- `--shadow-accent: 4px 4px 0px 0px var(--accent-500)` -- card accent
- `--shadow-accent-hover: 6px 6px 0px 0px var(--accent-700)` -- card hover
- `--shadow-focus-accent: 4px 4px 0px 0px var(--accent-500)` -- input focus

All shadow tokens have `.dark` overrides using `oklch(0.85 0 0)` for neobrutalist shadows.

#### 2.4 Dark Theme

- **Strategy:** `.dark` class on `<html>` via `next-themes` (`attribute="class"`)
- **Provider:** `ThemeProvider` in `apps/web/src/app/contexts/ThemeContext.tsx`
- **FOUC prevention:** Inline `<script>` in `index.html` reads `localStorage('theme')` before paint
- **Token override pattern:** All functional tokens in `tokens/color.css` have `.dark` selector overrides
- **Toggle:** `ThemeToggle` component in `apps/web/src/app/components/ui/theme-toggle.tsx`; `ThemeSelector` in settings

#### 2.5 Motion System

Duration and easing tokens in `tokens/motion.css`:
- Durations: `--duration-instant` (75ms) through `--duration-glacial` (800ms)
- Easings: `--ease-standard`, `--ease-decelerate`, `--ease-accelerate`, `--ease-sharp`, `--ease-editorial`, `--ease-spring`
- Intent tokens: `--intent-reveal`, `--intent-handoff`, `--intent-confirm`, `--intent-error`
- Reduced motion: All durations set to `0ms` via `@media (prefers-reduced-motion: reduce)`
...
- 禁止在组件 `.tsx` 文件中使用裸 hex 值；所有颜色必须引用 `var(--*)` 令牌。
- 动画 `transition-all` 禁止使用；必须指定具体属性（如 `transition-[transform,box-shadow,opacity]`）。
